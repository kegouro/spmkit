...
|---|---|---:|---|---|
| Gwyddion roughness 48 v0.1 | Sa, Sq, Sz on 48 canonical synthetic matrices | 144/144 within tolerance | `LEVEL 3 — CROSS_VALIDATED` | No preprocessing; shared matrices; not physical validation |
| Real-data roughness pilot v0.1 | Sa, Sq, Sz on 12 public GWY matrices | 36/36 shared-matrix comparisons within tolerance | `LEVEL 3` for the algorithm track | Parser/end-to-end observations are separate; real data are not ground truth |
| Nanoscope `.spm` pilot v0.1 | Six demonstrated files | 18/18 metric comparisons within tolerance | `LEVEL 2` limited parser claim | Partial support and `ACCIDENTAL_PRE_FREEZE_UNBLINDING` |

See [Scientific status](SCIENTIFIC_STATUS.md) for the complete mapping and
