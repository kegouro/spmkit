...
class MapViewModel(QObject):
    ...
    # ---- cálculo ----
    def compute_now(self, parallel: bool = False) -> None:
        """Calcula el mapa de forma sincrónica (tests / volúmenes chicos)."""
        volume = self._force_vm.volume
        if volume is None:
            return
        self._result = analyze_volume(volume, self._force_vm.recipe, parallel=parallel)
        self.mapReady.emit(self._result)

    def compute(self, parallel: bool = False) -> None:
        """Calcula el mapa en un hilo worker; emite progreso y ``mapReady`` al terminar.

        ponytail: ``parallel=False`` por defecto; para mapas de miles de curvas activar
        el paralelo (ProcessPoolExecutor) desde un control de la UI.
        """
        volume = self._force_vm.volume
        if volume is None:
