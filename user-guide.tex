% SPM-Kit {\textperiodcentered} Fathom Usage Guide --- v0.1.4 (released) | dev 0.1.5.dev0
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small SPM-Kit {\textperiodcentered} Fathom --- Usage Guide v0.1.4 / dev 0.1.5.dev0}
...
{\large Version 0.1.4 \,(Alpha) {\normalsize\,---\,in-tree \texttt{0.1.5.dev0}}}\\[0.3cm]
{\large 2026-07-27}\\[0.3cm]
{\large Released wheel: \texttt{0.1.4} \,(DOI \texttt{10.5281/zenodo.21303280})}\\[2cm]

{\normalsize
  \textbf{Author:} Jos\'e Labarca Baeza\\
  \textbf{License:} MIT\\
  ...
  contain additional detail. Both are verified against SPM-Kit~0.1.4 (development tree
  \texttt{0.1.5.dev0}).}
  \\[0.6cm]
  \emph{Acknowledgements:} Tom\'as Corrales and the SPM Lab at UTFSM provided selected
  experimental datasets and laboratory context during development and evaluation.
  Mar\'ia Saavedra Fredes and Benjamin Schleyer helped locate and share candidate
  datasets for the validation campaign. Developed without dedicated institutional funding.
}
...
SPM-Kit was developed independently by Jos\'e Labarca Baeza at Universidad T\'ecnica Federico Santa Mar\'ia (UTFSM), informed by practical work with AFM/SPM data, and is released under the MIT license.
...
\subsection{Status}
SPM-Kit is \textbf{alpha-stage} (Development Status~3). The \texttt{.nid} image parsers are
cross-checked against Gwyddion at machine precision --- this is \emph{parser parity} with
another implementation of the same formula, not physical-model validation. The roughness
parameters Sa, Sq and Sz are independently \textbf{cross-validated} against Gwyddion~2.71
(\textbf{LEVEL~3}, 18/18 comparisons over six synthetic surfaces); KPFM, spectral and the
remaining image analyses are exercised by unit and synthetic-recovery tests (\textbf{LEVEL~1/2}).
Force-spectroscopy workflows are tested on synthetic and lab data but have \emph{not} been
independently cross-validated. Nanoscope~III \texttt{.spm} image reading is numerically verified
against Gwyddion~2.71 at matrix precision (\textbf{LEVEL~2}, partial). Some readers
(\texttt{afmformats}, JPK TIFF, Bruker \texttt{.00N}) remain experimental (see \S\ref{sec:validation}).
...
\begin{lstlisting}[language=bash]
spmkit --version          # spmkit 0.1.4 (in-tree install reports 0.1.5.dev0)
...
.spm & Bruker/Nanoscope III & Images & Read & \textbf{Partial (LEVEL 2)} \\
...
The physical conversion $\mathit{phys} = \mathrm{Dim2Min} + (raw + 2^{31})/2^{32}\cdot\mathrm{Dim2Range}$
has been cross-checked against Gwyddion exports at \textbf{correlation~1.000000} for
topography, phase, and sensor channels. This is \emph{parser/conversion parity} with
another implementation of the same formula --- it does not, by itself, validate a physical
model. The independently cross-validated quantities at defined tolerance are the ISO~25178
roughness parameters Sa, Sq and Sz (LEVEL~3, vs Gwyddion~2.71, 18/18 over six synthetic
surfaces). See \texttt{docs/TRACEABILITY.md} and the external validation register.
...
SPM-Kit uses a layered approach and reports evidence with an explicit level vocabulary.
``Validated'' is not used as a blanket label: a capability is described by the strongest
evidence that actually applies to it.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Cross-checks} --- compare against Gwyddion for the same input (parser/conversion parity).
  \item \textbf{Instrument-file loading} --- real lab files load without errors.
  \item \textbf{Numerical verification (LEVEL~2)} --- matrix-precision agreement with an
        independent reference on a declared set of files (e.g.\ Nanoscope~III \texttt{.spm},
        6 files vs Gwyddion~2.71).
  \item \textbf{Cross-validation (LEVEL~3)} --- externally registered campaign comparing
        derived quantities at defined tolerance (currently Sa, Sq, Sz vs Gwyddion~2.71;
        18/18 over six synthetic surfaces).
  \item \textbf{Experimental} --- exists but lacks thorough testing or independent cross-check.
\end{enumerate}

\subsection{Evidence-level vocabulary}
\begin{description}[nosep]
  \item[LEVEL 1 (Unit tested)] Function-level tests pass on synthetic input.
  \item[LEVEL 2 (Numerically verified)] Matrix-precision agreement with an independent
        reference on a declared file set.
  \item[LEVEL 3 (Cross-validated)] Externally registered campaign comparing derived
        quantities at a defined tolerance, with a frozen script and frozen data set.
  \item[Experimental] Reader or analysis exists without independent cross-check.
\end{description}
...
\midrule
.nid image parsing & vs Gwyddion: corr 1.000000 & \textbf{Cross-checked} & Parser parity; signed 32-bit LE; other dtypes untested \\
.nid physical conv. & 8 integrity checks & \textbf{Cross-checked} & Parser/conversion parity; assumes Dim2Range linear \\
.nid orientation & flipud for Y channels & \textbf{Cross-checked} & Parser parity; non-image channels as-is \\
Roughness Sa/Sq/Sz (synth.) & vs Gwyddion 2.71, 18/18, 6 surfaces & \textbf{Cross-validated (L3)} & Synthetic surfaces, plane-levelled \\
Roughness ISO 25178 (general) & Synthetic $\sigma$ recovery & Tested & Ssk/Sku and general surfaces: synthetic-recovery only \\
Nanoscope III .spm & vs Gwyddion 2.71, 6 files & \textbf{Numerically verified (L2)} & Partial; scaling unresolved returns raw \\
...
Fathom GUI & Headless GUI tests (\texttt{tests/gui}) & Tested & Qt version/platform dependent \\
...
truth. Both tools implement their own conversion from the same byte stream. The cross-validated
campaign (LEVEL~3, Sa/Sq/Sz) is externally registered with a frozen script and frozen data set
at \url{https://github.com/kegouro/spmkit-validation}; it does not constitute physical-model
validation.
...
\textbf{Alpha (released \texttt{0.1.4}; in-tree \texttt{0.1.5.dev0}).} Core functionality is
stable and tested. APIs may change before 1.0. New contributors and format readers are welcome.
...
\begin{center}
{\small --- End of SPM-Kit {\textperiodcentered} Fathom Usage Guide v0.1.4 (dev 0.1.5.dev0) ---}
\end{center}
